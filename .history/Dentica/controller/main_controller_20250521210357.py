...
class MainController(QMainWindow, Ui_MainWindow):
    ...
    def update_patients_list(self, patients):
        ...
        for patient in patients:
            ...
            for col, value in enumerate(patient[1:7]):  # Columns 0-5
                item = QtWidgets.QTableWidgetItem(str(value))
                item.setFlags(item.flags() & ~QtCore.Qt.ItemFlag.ItemIsEditable)
                self.Patients_table.setItem(row_position, col, item)
            
            # Add empty item for column 6 to set background
            bg_item = QtWidgets.QTableWidgetItem()
            bg_item.setFlags(bg_item.flags() & ~QtCore.Qt.ItemFlag.ItemIsEditable)
            self.Patients_table.setItem(row_position, 6, bg_item)
